					...
					<div class="panel panel-default">
						<table class="table">
						    <tr>
								...
								<td align="center"><strong>Puntuación del Usuario</strong></td>
							...
							while ($tabla = mysql_fetch_array($result)){
								$query2 = mysql_query("SELECT AVG(puntaje) AS promedio FROM puntaje_huesped WHERE id_usuario = $tabla[id_huesped]");
							...
							<tr>
								...
								<?php 
									...
									if ($tabla["estado"] == 0){
									?>									
										<td align="center" style="vertical-align:middle;"><a href="reservation_acept.php?id_reserva=<?php echo htmlspecialchars($tabla["id_reserva_propiedad"]);?>"><button type="submit" name="aceptar" id="aceptar" onclick="return confirm('Esta seguro que desea aceptar esta reserva?')" class="btn btn-success btn-group-xs">Aceptar</button></a></td>
										<td align="center" style="vertical-align:middle;"><a href="reservation_cancel.php?id_reserva=<?php echo htmlspecialchars($tabla["id_reserva_propiedad"]);?>"><button type="button" name="rechazar" id="rechazar" onclick="return confirm('Esta seguro que desea rechazar esta reserva?')" class="btn btn-danger btn-group-xs">Rechazar</button></a></td>
									<?php
									}
									elseif ($tabla["estado"] == 2){
										if ($count == 0){
											...
											<div class="modal fade" id="myModal<?php echo htmlspecialchars($id_reserva);?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
												<div class="modal-dialog" role="document">
											    	<div class="modal-content">
											      		<form name="puntuacion" action="punctuation.php" method="POST" onsubmit="return validation()" enctype="multipart/form-data">
									...
									if ($tabla["estado"] > 1){
									?>
										<td align="center" style="vertical-align:middle;"><a href="reservation_delete.php?id_reserva=<?php echo htmlspecialchars($tabla["id_reserva_propiedad"]);?>"><button type="button" name="eliminar" id="eliminar" onclick="return confirm('Esta seguro que desea eliminar esta reserva del listado?')" class="btn btn-danger btn-group-xs">Eliminar</button></a></td>
									<?php
									}
									?>
								</tr>
